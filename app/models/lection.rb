class Lection < ApplicationRecord
  ...
  validates :title,length: {maximum: 255}, presence: true, uniqueness: { scope: :user_id, message: "must be unique for each of your posts" }
  validates :body,presence: true,length: {maximum: 45000},on: :create,allow_nil: false
  validates :speciality, length: {maximum: 20}
